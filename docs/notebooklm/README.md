# NotebookLM 文件包

這一組文件是專門為 `NotebookLM`、簡報生成與 Demo 影片腳本整理的來源資料。和一般工程文件不同，這裡更強調敘事順序、角色流程、技術亮點與可直接講出口的內容。

## 建議匯入順序

1. `project-overview.md`
2. `business-flows.md`
3. `technical-deep-dive.md`
4. `presentation-script.md`

## 各文件用途

- `project-overview.md`
  給 NotebookLM 建立整體專案理解，適合作為簡報主架構來源。
- `business-flows.md`
  讓模型理解角色差異、操作流程與 Demo 的故事線。
- `technical-deep-dive.md`
  補足架構、驗證、資料模型、背景服務與測試等工程細節。
- `presentation-script.md`
  直接提供 5 到 10 分鐘簡報或影片解說腳本素材。

## 使用建議

- 如果要做「面試簡報」，優先使用 `project-overview.md` 與 `presentation-script.md`
- 如果要做「產品流程型影片」，再加上 `business-flows.md`
- 如果要做「技術導向簡報」，把 `technical-deep-dive.md` 一起餵進去

## Demo 固定資料

- API 主要入口：`/swagger/index.html`
- 共同密碼：`P@ssw0rd!`
- 管理員帳號：`E0001`
- 教師帳號：`T0001`
- 家長帳號：`0900-000-003`
- 學生帳號：`S10001`
- 建議示範週次：`2026-03-16` 這一週
